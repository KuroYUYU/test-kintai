...
@section('content')
<div class="index">
    <h1 class="index__title">{{ $user->name }}さんの勤怠</h1>

    {{-- 月表示切り替えナビ --}}
    <div class="attendance-list__month-card">
        <div class="index__month-nav">
            <a href="{{ route('admin.attendance.staff', ['user' => $user->id, 'month' => $previousMonth]) }}">
    ...
    {{-- CSV出力 --}}
    <div class="index__csv-actions">
        <a href="{{ route('admin.attendance.staff.csv', ['user' => $user->id, 'month' => $month->format('Y-m')]) }}" class="index__csv-button">CSV出力</a>
    </div>
</div>
@endsection